...
describe('todo delete command', () => {
  ...
  it('should delete a task', () => {
    createTaskStore([
      { id: 1, description: 'Task to delete', status: 'pending' },
      { id: 2, description: 'Task to keep', status: 'pending' },
    ]);

    const result = runCli('delete 1');

    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Deleted task #1');
    expect(result.stdout).toContain('Task to delete');
  ...
  it('should delete a completed task', () => {
    createTaskStore([{ id: 1, description: 'Completed task', status: 'completed' }]);
  ...
  it('should output JSON with --json flag', () => {
    ...
    const result = runCli('delete 1 --json');
  ...
  it('should output JSON error for not found', () => {
    const result = runCli('delete 999 --json');
